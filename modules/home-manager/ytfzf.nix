...
in
{
	options.ytfzf = {
		...
		variables = lib.mkOption {
			type = with lib.types; attrsOf (oneOf [ints.unsigned str bool]);
			default = {
	...
	config = lib.mkIf cfg.enable {
		...
		xdg.configFile."ytfzf/conf.sh".text = lib.mkIf (cfg.variables != {} || cfg.functions != {} || cfg.extraConfig != "") settingsFile;
	};
}
