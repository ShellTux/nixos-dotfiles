...
in
{
	options.apps.cli.yazi.enable = lib.mkEnableOption "Enable yazi module";
		...
		programs.yazi = {
			...
			keymap = {
				manager = {
					...
					{ on = [ "x"  "y" ];    run = "yank --cut";                 desc = "Cut the selected files"; }
					{ on = [ "x"  "u" ];    run = "unyank";                     desc = "Cancel the yank status of files"; }
					...
					prepend_keymap = [
					{ on = [ "<A-s>" ]; run  = ''shell "$SHELL" --block --confirm''; desc = "Open shell here"; }
					];
				};
			...
			settings = {
				...
				{ name = "*.md"; run = "${pkgs.glow}/bin/glow"; }
				];
			};
		};
	};
